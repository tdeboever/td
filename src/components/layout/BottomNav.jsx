    ...
    <nav className="safe-bottom" style={{
      ...
      <div className="flex items-center justify-around h-16" style={{ padding: '0 20px' }}>
        {TABS.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => tab.id === 'spaces' ? toggleSidebar() : setView(tab.id)}
              className="flex flex-col items-center gap-1 flex-1 py-1"
              style={{ color: active ? 'var(--accent-coral)' : 'var(--text-secondary)' }}>
              <span style={{ fontSize: 20, lineHeight: 1, textShadow: active ? '0 0 14px rgba(255,123,84,0.35)' : 'none' }}>{tab.icon}</span>
              <span style={{ fontSize: 10, fontWeight: 500 }}>{tab.label}</span>
              {active && <div style={{ width: 24, height: 3, borderRadius: 9999, background: 'linear-gradient(135deg, #ff7b54, #f472b6)', boxShadow: '0 0 10px rgba(255,123,84,0.35)' }} />}
            </button>
          )
        })}
      </div>
